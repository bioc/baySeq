...
getPriors.Pois <- function (cD, samplesize = 1e+05, perSE = 0.1, takemean = TRUE,
                                maxit = 1e+05, weights = NULL, verbose = TRUE, cl)
  {
    if (!inherits(cD, what = "countData")) 
        stop("variable 'cD' must be of or descend from class 'countData'")
    if (length(cD@libsizes) == 0) {
    ...
    priorPars <- function(seluu, initial, lensameFlag) {
        PgivenPois <- function(priors, us, lens, ns, lensameFlag) {
            ...
            alpha <- priors[1]
            beta <- priors[2]
            sum(alpha * log(beta) - lgamma(alpha) + rowSums(log(outer(as.integer(lens), 
                ns)) * us) - rowSums(lfactorial(us)) + lgamma(alpha + 
                rowSums(us)) - (alpha + rowSums(us)) * log(beta + 
                rowSums(outer(lens, ns))))
        }
        us <- 0
        ...
        if (all(!is.na(initial) & initial > 0 & initial < Inf)) {
            pars <- optim(initial, PgivenPois, control = list(fnscale = -1), 
                us = us, lens = lens, ns = ns, lensameFlag = lensameFlag)$par
            return(pars)
        }
        else return(c(NA, NA))
    }
    ...
    if (!is.null(cl)) {
        clustAssignData <- function(y, libsizes, samplesize, 
            weights) {
        ...
        clusterCall(cl, clustAssignData, y, libsizes, samplesize, 
            weights)
    }
    groups <- cD@groups
    priors <- lapply(1:length(groups), function(gg) {
      lapply(levels(groups[[gg]]), function(uu) {
        ...
        seluu <- which(groups[[gg]] == uu)
        for (rr in 1:maxit) {
          if (!is.null(tempPriors)) 
            initial <- apply(tempPriors, 2, mean, na.rm = TRUE)
          if (!is.null(cl)) {
            pars <- clusterCall(cl, priorPars, seluu = seluu, 
                                initial = initial, lensameFlag = lensameFlag)
            pars <- matrix(unlist(pars), ncol = 2, byrow = TRUE)
          }
          else {
            pars <- matrix(priorPars(seluu = seluu, initial = initial, 
                                     lensameFlag = lensameFlag), ncol = 2, nrow = 1)
                }
                tempPriors <- rbind(tempPriors, pars[apply(!is.na(pars), 
                  1, all), ])
                if (nrow(tempPriors) > 1) 
                  if (all((apply(tempPriors, 2, sd)/sqrt(nrow(tempPriors)))/apply(tempPriors, 
                    2, mean) < perSE)) 
                    break()
            }
            if (rr == maxit) 
